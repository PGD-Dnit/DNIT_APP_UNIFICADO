...
export default function SwipeView() {
  ...
  const leftTile = useAppStore((s) => s.leftTile);
  const rightTile = useAppStore((s) => s.rightTile);
  const position = useAppStore((s) => s.swipe.position);
  ...
  useEffect(() => {
    ...
    let leftLayer: WebTileLayer | null = null;
    let rightLayer: WebTileLayer | null = null;
    ...
    // cria tiles
    if (rightTile?.tileUrl) {
      rightLayer = new WebTileLayer({
        urlTemplate: rightTile.tileUrl,
        title: rightTile.title,
      });
      map.add(rightLayer);
    }

    if (leftTile?.tileUrl) {
      leftLayer = new WebTileLayer({
        urlTemplate: leftTile.tileUrl,
        title: leftTile.title,
      });
      map.add(leftLayer);
    }
    ...
    swipe = new Swipe({
      view,
      leadingLayers: leftLayer ? [leftLayer] : [],
      trailingLayers: rightLayer ? [rightLayer] : [],
      position,
    });
  ...
  }, [leftTile?.id, rightTile?.id, exposureLayerUrl]);
